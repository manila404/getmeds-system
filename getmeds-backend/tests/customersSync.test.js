...
describe('Customers — Zoho sync (read-only) and single-TEST-customer gate', () => {
  ...
  beforeAll(async () => {
    ...
    db.prepare(`DELETE FROM customers WHERE source = 'zoho'`).run();
  });
  ...
  test('POST /api/customers/sync-from-zoho pulls mock contacts into the local table without ever calling a Zoho write', async () => {
    ...
    const pulled = db.prepare(`SELECT * FROM customers WHERE source = 'zoho'`).all();
  ...
  test('POST /api/customers/sync-from-zoho is idempotent — running it again refreshes, does not duplicate', async () => {
    const before = db.prepare(`SELECT COUNT(*) as n FROM customers WHERE source = 'zoho'`).get().n;
    ...
    const after = db.prepare(`SELECT COUNT(*) as n FROM customers WHERE source = 'zoho'`).get().n;
  ...
  test('gate ON: order-creation dropdown shows only the designated TEST customer, and creating an order for anyone else is rejected', async () => {
    const zohoCustomer = db.prepare(`SELECT * FROM customers WHERE source = 'zoho' LIMIT 1`).get();
    ...
    const otherCustomer = db.prepare(`SELECT * FROM customers WHERE source = 'local' AND is_active = 1 LIMIT 1`).get();

...
| Sales Person | `orders.medrep_id` → `users.name` | `salesperson_id` (or `salesperson_name` in newer Zoho API versions) | **Needs a new mapping.** Zoho's field wants a Zoho salesperson ID, not a name. This app only has the Getmeds user's name today — a `users.zoho_salesperson_id` column (nullable) would need to be added and populated (e.g. from Zoho's salespersons list), and `orders.controller.js` would resolve `req.user`'s row to that ID before sending. Sending nothing here just leaves the Sales Order's salesperson blank in Zoho. |
